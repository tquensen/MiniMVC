    <input type="checkbox"
           ...
           <?php if ($element->value): ?> checked <?php endif; ?> /> <?php if ($element->label && (!isset($label) || $label !== false)): ?> <label
                      for="<?php echo htmlspecialchars($element->getForm()->getName() . '__' . $element->getName()) ?>"><?php echo htmlspecialchars($element->label) ?><?php if ($element->required && $element->getForm()->getOption('requiredMark')): ?><?php echo '<span class="requiredMark">'.htmlspecialchars($element->getForm()->getOption('requiredMark')).'</span>'; ?><?php endif; ?></label> <?php endif; ?>