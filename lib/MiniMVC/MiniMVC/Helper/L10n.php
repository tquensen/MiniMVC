...
class Helper_L10n extends MiniMVC_Helper
{
    ...
    {
        return date($this->getL10nData('formatDate', $language), $timestamp !== null ? (int) $timestamp : time());
    }

    public function formatTime($timestamp = null, $showSeconds = false, $language = null)
    {
        return date($this->getL10nData($showSeconds ? 'formatTimeSeconds' : 'formatTime', $language), $timestamp !== null ? (int) $timestamp : time());
    }

    public function formatDateTime($timestamp = null, $showSeconds = false, $language = null)
    {
        return date($this->getL10nData($showSeconds ? 'formatDateTimeSeconds' : 'formatDateTime', $language), $timestamp !== null ? (int) $timestamp : time());
    }
